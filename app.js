...
app.use((req, res, next) => {
  ...
  next();
  // ต้องเรียกเสมอ ไม่งั ้นค าขอจะค้าง ไปต่อไม่ถึง route
});
...
let tasks = [
  ...
  { id: 2, text: "ติดตั ้ง VS Code", done: true },
];
let nextId = 3;
...
// GET /api/tasks — ดึงรายการทั ้งหมด
...
app.get("/api/tasks/:id", (req, res) => {
  ...
  if (!task) {
    return res.status(404).json({ error: "ไม่พบรายการนี ้" });
  }
...
// POST /api/tasks — เพิ ่มรายการใหม่
